...
const Contact = () => {
  ...
  const renderWebsites = Array.isArray(website) ?
    website.map((web) => listItem('website', 'fas fa-globe', web?.url ?? '', true, web?.url, web?.url))
    :
    listItem('website', 'fas fa-globe', website?.url ?? '', true, website?.url, website?.url)
